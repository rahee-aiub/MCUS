...
CREATE PROCEDURE [dbo].[Sp_HRLeaveVerify](@userID INT)   
...
IF @Desig IN (7,10,11)
	BEGIN
		DELETE FROM A2ZHRMCUS..WFLEAVEAPPROVE WHERE EmpDesignation IN (1,3,4,5,6,7,10,11) AND UserID = @UserID
		...
		SELECT  EmpCode,EmpleaveCode,LStartDate,LEndDate,LApplyDays,LApplyDate,LBalance,LInputeBy,
			    LInputeDate,LCheckBy,LCkeckDate,Status,Porpus,Note
		FROM A2ZHRMCUS..A2ZEMPLEAVE,A2ZHRMCUS..WFLEAVEAPPROVE
		WHERE A2ZHRMCUS..A2ZEMPLEAVE.Status = 1 AND A2ZHRMCUS..A2ZEMPLEAVE.EmpCode = A2ZHRMCUS..WFLEAVEAPPROVE.EmpIdsNo 
