...
{
    public partial class CSAutoRenewal : System.Web.UI.Page
    {
        ...
        protected void ValidationProcess()
        {
            try
            {
                ...
                {
                    string qry1 = "SELECT Id,TrnDate,ProcStat,VoucherNo FROM WFCSRENEWFDR where Id = 1";
                    dt1 = DataAccessLayer.BLL.CommonManager.Instance.GetDataTableByQuery(qry1, "A2ZCSMCUS");
                }

                if (ddlAccType.SelectedValue == "16")
                {
                    string qry1 = "SELECT Id,TrnDate,ProcStat,VoucherNo FROM WFCSRENEW6YR where Id = 1";
                    dt1 = DataAccessLayer.BLL.CommonManager.Instance.GetDataTableByQuery(qry1, "A2ZCSMCUS");
                }
        ...
        {
            string sqlquery = "SELECT AccTypeCode,AccTypeDescription from A2ZACCTYPE Where AccTypeClass BETWEEN 2 AND 3";
        ...
        protected void gvPreview()
        {
            try
            {
                ...
                if (ddlAccType.SelectedValue == "15")
                {
                    string sqlquery3 = "SELECT CuNumber,MemNo,AccType,AccNo,MemName,FDAmount,NewIntRate,NoDays,CalInterest FROM WFCSRENEWFDR Where AccType='" + ddlAccType.SelectedValue + "' ORDER BY CuType,CuNo";

                    gvDetailInfoFDR = DataAccessLayer.BLL.CommonManager.Instance.FillGridViewList(sqlquery3, gvDetailInfoFDR, "A2ZCSMCUS");
                }
                if (ddlAccType.SelectedValue == "16")
                {
                    string sqlquery3 = "SELECT CuNumber,MemNo,AccType,AccNo,MemName,FDAmount,NewIntRate,NoDays,CalInterest FROM WFCSRENEW6YR Where AccType='" + ddlAccType.SelectedValue + "' ORDER BY CuType,CuNo";
        ...
        protected void BtnCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlAccType.SelectedValue == "15")
                {
                    int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("Sp_CSCalculateRenewalFDR", prm, "A2ZCSMCUS"));
                    if (result == 0)
                    {
                ...
                if (ddlAccType.SelectedValue == "16")
                {
                    int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("Sp_CSCalculateRenewal6YR", prm, "A2ZCSMCUS"));
                    if (result == 0)
                    {
        ...
        protected void BtnReverse_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlAccType.SelectedValue == "15")
                {
                    int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("Sp_CSDeleteRenewalFDR", prm, "A2ZCSMCUS"));
                    if (result == 0)
                    {
                ...
                if (ddlAccType.SelectedValue == "16")
                {
                    int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("Sp_CSDeleteRenewal6YR", prm, "A2ZCSMCUS"));
                    if (result == 0)
                    {
